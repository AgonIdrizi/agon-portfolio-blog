...
Building a ThemeProvider or AuthProvider is common usage of Context-api.

In this guide we'll demonstrate in which situations ContextProvider re-renders and how it affects it's children.
...
Now let's try something, if we click the Count button, state inside App will change and App will re-render, making all it's children components to re-render.

When parent component of a provider re-renders, then ProviderComponent will re-render and all it's children too, this is default behaviour!
...
re-renders, all it's children will re-render, even if they are wrapped and optimized with
React.memo**.
...
Thus making it's children to re-render!
...
Redux uses Context under the hood and they optimize Providers too, but that's for some other post:). Hope this will help someone!
